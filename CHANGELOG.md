...
- **Delete/Trash modal shows only one item when multiple selected**: When multiple files or folders were marked and F8 (Trash) or Shift+F8 (Delete) was pressed, the confirmation modal only displayed the first marked item, making it appear as though only one item would be affected — even though the operation correctly deleted all of them. The modal now lists all marked items by filename (up to 5, with "... and N more" if there are more), and dynamically grows taller to fit the list.
